...
pub fn insert_subscriptions(list_name: &str, 
  ...
  let prep_get_list_stmt = transaction.prep(get_list_stmt).context(error::DbPrepareError { statement: get_list_stmt })?;
  let list_id : i32 = transaction.exec_first(&prep_get_list_stmt, params! { "email" => list_name })
  ...
  let insert_result = transaction.exec_batch(insert_statement, 
    subscriptions.iter().map(|s| params! { "uuid" => s.uuid.clone(),
                                  "list_id" => list_id,
                                  "email" => s.email.clone(),
                                  "request" => request,
  }));
  if let Err(err) = insert_result {
    ...
    return Err(err).context(error::DbExecuteError { statement: insert_statement });
  }
