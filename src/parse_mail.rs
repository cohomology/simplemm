...
pub fn get_addresses_in_from_header(from_header: &str) -> std::vec::Vec<String> {
    ...
    REGEX.find_iter(from_header).map(|elem| elem.as_str().to_string()).collect() 
}
...
mod tests {   
    #[test]
    fn parse_mails() {
        assert_eq!(super::get_addresses_in_from_header("foo@bar, Frank Rotzelpü <frank@göckel.com>, Murkel <\"my murkel\"@localhost"),
